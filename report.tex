\documentclass[a4paper,11pt]{article}
\usepackage{amssymb}
\usepackage{booktabs}
\usepackage{geometry}
\usepackage[hidelinks]{hyperref}
\usepackage{listings}
\usepackage{lipsum}
\usepackage{graphicx}
\usepackage{natbib}

\bibliographystyle{plain}

\geometry{
	includeheadfoot,
	margin=2.54cm
}

\title{
	2IV55 Interactive Virtual Environments: Initial Report Group 2
}
\author{
	Tim van Dalen (0744839)
	\and
	Robbert Jongeling (0747896)
	\and
	Jasper Selman (0741516)
	\and
	Ramon de Vaan (0758873)
	\and
	Bart van Wezel (0740608)
}
\date{\today}

\begin{document}
	\maketitle
	
	\begin{abstract}
		The goal of our project is to investigate redirected walking. 
		In particular, we are interested in the so called rotation, translation and curvature ``gains" and how those differ when a user is engaged in a task while walking versus when a user is just aimlessly walking around.
	\end{abstract}
	
	\section{Problem definition}
	\label{sec:problem}
	\input{sections/problem}
	
	\section{Virtual Reality Concepts}
	\label{sec:vr}
	\input{sections/vr}
	
	\section{Approach}
	\label{sec:approach}
	\input{sections/approach}
	
	\bibliography{bibliography}
\end{document} 
